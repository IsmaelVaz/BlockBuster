         ...
         if FSocio.TemLocPendente = False then begin
            if FObject.Itens.Count>0 then begin
               ...
               if FraseFilmeSemQtd='' then begin
                  ...
                  Application.MessageBox('Locação Aprovada.','Aviso',MB_ICONINFORMATION);

                  UpdateScreen(false);
                  exit;
               end else begin
                  Application.MessageBox(PChar('Os itens abaixo não tem quantidade suficiente em estoque.'+FraseFilmeSemQtd),'Aviso',MB_ICONINFORMATION);
                  exit;
               end;
            end else begin
               Application.MessageBox('É preciso ter ao menos um item na Locação.','Aviso',MB_ICONINFORMATION);
               exit;
            end;
         end else begin
            Application.MessageBox('O Sócio possuí locação pendente'+#13+'Finalize a Locação pendente antes de iniciar uma nova.','Aviso',MB_ICONINFORMATION);
            exit;
         end;
      end;
   finally
...
begin
   ...
   if (ValidateScreen) then begin
      if (FObject.StatusLocacao = 'A') then begin
         ...
         Application.MessageBox('Locação iniciada.','Aviso',MB_ICONINFORMATION);
         UpdateScreen(false);
         exit;
      end else begin
         Application.MessageBox('É preciso aprovar a Ficha antes de iniciar a Locação','Aviso',MB_ICONINFORMATION);
         exit;
      end;
   end;
end;
...
begin
   ...
   if (ValidateScreen) then begin
      if (FObject.StatusLocacao = 'L') then begin
         ...
         Application.MessageBox('Locação Finalizada.','Aviso',MB_ICONINFORMATION);
         UpdateScreen(False);
         exit;
      end else begin
         Application.MessageBox('A ficha não está em locação','Aviso',MB_ICONINFORMATION);
         exit;
      end;
   end;
end;
...
//$$** SECTION: UPDATESCREEN_USER
   //Se estiver em locação, desativa botão de Aprovar e o de Iniciar a Locação
   ...
   //Se estiver Aprovado, desativa botão de Aprovar e o de Finalizar a Locação
   ...
   //Se estiver Finalizado, desativa botão de Aprovar, Iniciar e Finalizar a Locação
   ...
   //Se estiver Em Preparação, desativa botão de Iniciar e Finalizar a Locação
...
//$$** SECTION: VALIDATESCREEN_BODY
   ...
   if (cboSocio.ItemIndex=-1) or (cboSocio.Items.Count=0) then begin
      Application.MessageBox('O campo "Sócio" deve ser preenchido.','Aviso',MB_ICONINFORMATION);
   ...
   if txtDataInicial.Text<>'  /  /    ' then begin
      try
         ...
         Application.MessageBox('O valor do campo "Data Inicial" é inválido.','Aviso',MB_ICONINFORMATION);
   ...
   if txtDataFinal.Text<>'  /  /    ' then begin
      try
         ...
         Application.MessageBox('O valor do campo "DataFinal" é inválido.','Aviso',MB_ICONINFORMATION);
         if txtDataFinal.CanFocus then txtDataFinal.SetFocus;
   ...
   try
      ...
      Application.MessageBox('O valor do campo "Desconto" é inválido.','Aviso',MB_ICONINFORMATION);
   ...
   try
      ...
      Application.MessageBox('O valor do campo "Acréscimo" é inválido.','Aviso',MB_ICONINFORMATION);
...
//$$** SECTION: CUSTOMVALIDATESCREEN_BODY
   try
      ...
      begin
         Application.MessageBox('A Data Final não pode ser menor que a Data Inicial.','Aviso',MB_ICONINFORMATION);
         if txtDataFinal.CanFocus then txtDataFinal.SetFocus;
         if txtDataFinal.CanFocus then txtDataFinal.SelectAll;
         Result:=False;
         Exit;
